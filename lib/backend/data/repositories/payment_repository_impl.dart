...
import 'package:poultry_accounting/backend/domain/entities/customer.dart' as customer_domain;
import 'package:poultry_accounting/backend/domain/entities/payment.dart' as domain;
import 'package:poultry_accounting/backend/domain/entities/supplier.dart' as supplier_domain;
...
class PaymentRepositoryImpl implements PaymentRepository {
  ...
  Future<int> createReceipt(domain.Payment payment) async {
    return database.transaction(() async {
      ...
      await accountingRepo.createPaymentReceivedJournalEntry(
        paymentId: id,
        paymentNumber: paymentNumber,
  ...
  Future<int> createPayment(domain.Payment payment) async {
    return database.transaction(() async {
      ...
      // --- ACCOUNTING INTEGRATION ---
      final accountingRepo = AccountingRepositoryImpl(database);
      await accountingRepo.createPaymentMadeJournalEntry(
        paymentId: id,
        paymentNumber: paymentNumber,
        userId: payment.createdBy ?? 1,
        amount: payment.amount,
