  ...
  Future<void> confirmInvoice(int invoiceId, int userId) async {
    await database.transaction(() async {
      ...
      if (confirmedInvoice != null) {
        ...
        for (final item in confirmedInvoice.items) {
          totalItemCost += item.costAtSale * item.quantity;
        }
        await accountingRepo.createSaleJournalEntry(
          saleId: confirmedInvoice.id!,
          saleNumber: confirmedInvoice.invoiceNumber,
          userId: userId,
          totalAmount: confirmedInvoice.total,
          totalCost: totalItemCost,
          amountPaid: confirmedInvoice.paidAmount,
          customerId: confirmedInvoice.customerId,
          discountAmount: confirmedInvoice.discount,
          saleDate: confirmedInvoice.invoiceDate,
        );
      }
    });
  }
  ...
  Future<void> cancelInvoice(int invoiceId, int userId) async {
    await database.transaction(() async {
      ...
      if (invoice.status == InvoiceStatus.confirmed) {
        ...
        await accountingRepo.createSaleVoidJournalEntry(
          saleId: invoice.id!,
          saleNumber: invoice.invoiceNumber,
          userId: userId,
